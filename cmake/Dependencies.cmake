...
if(NOT CMAKE_SYSTEM_PROCESSOR MATCHES "^(s390x|ppc64le)$")
  ...
  if(NOT TARGET cpuinfo AND USE_SYSTEM_CPUINFO)
    ...
    message("Found cpuinfo: ${CPUINFO_LIBRARY_DIR}")
...
# ---[ cuDNN
# @EUGO_CHANGE - ability to use system cudnn frontend
if(USE_CUDNN)
  if(NOT TARGET cudnn)
    if (USE_SYSTEM_CUDNN)
        if (CUDNN_FRONTEND_INCLUDE_DIR)
          message(STATUS "Using CUDNN Frontend specified at ${CUDNN_FRONTEND_INCLUDE_DIR}.")
          list(APPEND CMAKE_PREFIX_PATH ${CUDNN_FRONTEND_INCLUDE_DIR})
        endif()
        find_package(cudnn_frontend 8.3.0)
        if(NOT cudnn_frontend_VERSION VERSION_GREATER_EQUAL 8.3.0)
          message(FATAL_ERROR "Found cuDNN Frontend version ${cudnn_frontend_VERSION}, but version 8.3.0 or greater is required.")
        endif()
        if(NOT cudnn_frontend_FOUND)
          message(FATAL_ERROR "Cannot find System CUDNN Frontend")
        endif()
        message("-- Found CUDNN Frontend: ${cudnn_frontend_DIR}")
        # @HELP - needed?
...
if(NOT INTERN_BUILD_MOBILE)
  ...
  # @EUGO_CHANGE - ability to use system kleidiai (see https://github.com/google/XNNPACK/blob/master/CMakeLists.txt#L1242-L1261 for inspiration)
  if(USE_KLEIDIAI)
    if(NOT TARGET kleidiai)
      if(USE_SYSTEM_KLEIDIAI)
        add_library(kleidiai SHARED IMPORTED)
        # TODO+ define KLEIDIAI_SOURCE_DIR for system kleidiai so user's dont have to pass `KLEIDIAI_SOURCE_DIR`? like `if (KLEIDIAI_SOURCE_DIR) \ list(APPEND CMAKE_PREFIX_PATH ${KLEIDIAI_SOURCE_DIR})
        find_library(KLEIDIAI_LIBRARY kleidiai PATHS "${KLEIDIAI_SOURCE_DIR}/lib")
        if(NOT KLEIDIAI_LIBRARY_FOUND)
          message(FATAL_ERROR "Cannot find System KleidiAI")
        endif()
        message("-- Found system kleidiai: ${KLEIDIAI_LIBRARY_DIR}")
